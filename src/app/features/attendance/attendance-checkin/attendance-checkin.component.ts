...
export class AttendanceCheckinComponent implements OnInit {
  ...
  todayStr = new Date().toLocaleDateString('en-IN', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
  ...
  load() {
    ...
    this.attService.getMyAttendance().subscribe({
      next: att => {
        this.attendance.set([...att].reverse());
        const today = new Date().toDateString();
        this.todayRecord.set(att.find(a => new Date(a.date).toDateString() === today) ?? null);
        this.loading.set(false);
      },
      error: () => { this.loading.set(false); this.toastService.error('Failed to load attendance.'); }
    });
  }
...
// Add this method inside the class (append)
